...
export const ProcessSearchModal: React.FC<ProcessSearchModalProps> = ({ isOpen, onClose, apiKey }) => {
  ...
  const buscarProcesso = async () => {
    ...
    const query = {
      query: {
        match: {
          numeroProcesso: numeroProcesso.trim().replace(/\D/g, '') // Remove non-digits
        }
      }
    };
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (axiosError.message.includes("Network Error")) {
          ...
          if (axiosError.response.status === 401) {
            errorMessage = "Erro de autenticação (401): Verifique a chave da API DataJud fornecida.";
          }
        } else {
      ...
      <div className="modal-content custom-scrollbar" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={isLoading}
            />
             <p className="text-xs text-gray-400 mt-1">Ex: 0710802-55.2018.8.02.0001</p>
          </div>
          <button
        ...
        {error && (
          <div className="mt-4 p-3 bg-red-900/30 border border-red-700 rounded-md">
            <p className="text-red-300 text-sm font-semibold">Erro na Consulta:</p>
        ...
        {resultado && (
          <div className="mt-6">
            <h3 className="text-xl font-semibold text-gray-200 mb-2">Resultado da Consulta:</h3>
             {resultado.hits?.total?.value === 0 && <p className="text-gray-400">Nenhum processo encontrado com o número fornecido.</p>}
